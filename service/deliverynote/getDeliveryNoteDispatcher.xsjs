	...
	{ 
	    ...
	    if(codvehiculo !== undefined){
            var query = 'SELECT \'A\' AS "Tipo", ' +
                          ...
                          '  	WHERE (T0."DocTotal" <> T0."PaidToDate" OR T0."DocTotal" = 0)' +
                          '  	AND T0."CANCELED" = \'N\' ' +
                          '  AND IFNULL(T0."FolioNum",0) <> 0 ' +
                          '  AND T0."U_MSS_FETR" IS NOT NULL ' + 
                          '  AND TO_VARCHAR(T0."U_MSS_FETR",\'YYYYMMDD\') = CURRENT_DATE ' +
    	    ...
    	    objType = "MessageError";
    	    objResult = functions.CreateJSONMessage(-99, "El usuario no tiene un vehiculo asignado. U("+empId+")");
    	    objResponse = functions.CreateResponse(objType, objResult, 0);
    	    functions.DisplayJSON(objResponse, objType);
    	}
