...
function ValidarClaveMovil(ClaveMovil){
	try	{
		var mQuery = 'select count(*) as "last" from '+Constants.BD_MOBILE+'."ORDN" ' +
...
function RegistrarCabecera(ordn){
    ...
    try{
        ...
        var query = 'insert into '+Constants.BD_MOBILE+'."ORDN" values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
...
function RegistrarLotes(Lotes){
    ...
    try{
		...
		{
		    var query = 'insert into '+Constants.BD_MOBILE+'."RDN2" values(?,?,?,?)';
            var pstmt = conn.prepareStatement(query);   
...
function RegistrarDetalle(ClaveMovil,lines){
    ...
    try{
		...
		{
		    var query = 'insert into '+Constants.BD_MOBILE+'."RDN1" values(?,?,?,?,?,?,?,?,?,?,?)';
            var pstmt = conn.prepareStatement(query);   
		...
		if(messageValidation === Constants.MESSAGE_SUCCESS){
		    ...
		    if(resCabe === Constants.MESSAGE_SUCCESS){
	            ...
	            if(resDet === Constants.MESSAGE_SUCCESS){
			        ...
			        functions.AddLogRegister(ordnCabe.Empresa, ordnCabe.EmpleadoVenta, ordnCabe.ClaveMovil, 
