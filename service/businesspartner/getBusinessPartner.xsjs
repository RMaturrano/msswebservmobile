...
try{
	...
	{ 
	    var locQuery = '	T0."U_MSSL_BTP" AS "TipoPersona", ' +
        ...
        if(functions.isEnabledLocalization(empId) === 'N'){
            locQuery = '';
        }
                        
	    var dbname = functions.GetDataBase(empId);
        var query = 'SELECT LTRIM(RTRIM(T0."CardCode")) AS "Codigo",  ' +
                     ...
                     //' AND T1."CardCode" = T0."CardCode"  AND T1."AdresType" = \'B\' AND T1."U_MSS_COVE" = \'' +cove+ '\' ' +
                     ' AND T1."CardCode" = T0."CardCode"  AND T1."AdresType" = \'B\' '+
                     '   WHERE T0."CardType" <> \'S\' ' + 
                     '   AND  (SELECT IFNULL(COUNT("U_MSS_COVE"),0) FROM ' + dbname + '.CRD1 WHERE ' +
                     ' "CardCode" = T0."CardCode" AND "AdresType" = \'B\' AND "U_MSS_COVE" = \'' +cove+ '\' ) > 0 ' + 
                     //'   AND T0."validFor" = \'Y\' '+
                     '   AND T0."frozenFor" = \'N\''; 
	    ...
	    if (rs.length > 0)
    	{
    		...
    		{
    		    try{
    		        var dataSales; // Los montos se muestran a nivel de direccion
    		    
        			mBusinessPartner = '{';   
        			mBusinessPartner += '"Codigo": "'+functions.ReplaceInvalidChars(rs[i].Codigo)+'",';
            		...
            		mBusinessPartner += '"NumeroDocumento": "'+functions.ReplaceInvalidChars(rs[i].NumeroDocumento)+'",';
            		mBusinessPartner += '"NombreRazonSocial": "'+functions.ReplaceInvalidChars(rs[i].NombreRazonSocial)+'",';
            		mBusinessPartner += '"NombreComercial": "'+functions.ReplaceInvalidChars(rs[i].NombreComercial)+'",';
            		...
            		mBusinessPartner += '"CorreoElectronico": "'+functions.ReplaceInvalidChars(rs[i].CorreoElectronico)+'",';
