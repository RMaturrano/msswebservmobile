...
try{
	...
	{ 
	    ...
	    var codvehiculo = functions.GetCodAuto(dbname, usrId);
	    ...
	    if(codvehiculo !== undefined){
            var query =   'SELECT     ' +
                              ...
                              '  AND T0."CANCELED" = \'N\' ' +
                             /* '  AND T0."DocDueDate" IS NOT NULL ' + 
                              '  AND TO_VARCHAR(T0."DocDueDate",\'YYYYMMDD\') = \'' + functions.GetCurrentDate() + '\' ' +
                              '  AND T0."U_MSSL_PVH" = \'' + codvehiculo + '\''; */
                              '     AND T0."CardCode" IN (SELECT X0."CardCode" FROM '+ dbname + '.ODLN  X0 ' +
                          '                           WHERE X0."DocType" = \'I\' ' + 
                          //'                             AND IFNULL(X0."U_MSSM_CRM",\'N\') = \'Y\' ' +
                          //'                           AND IFNULL(X0."U_MSSM_TRM",\'01\') <> \'01\' ' + 
                          '                           AND X0."CANCELED" = \'N\' ' + 
                          '                           AND X0."DocDueDate" IS NOT NULL ' + 
                          '                           AND TO_VARCHAR(X0."DocDueDate",\'YYYYMMDD\') = CURRENT_DATE ' +
                          //'                           AND X0."SlpCode" <> -1  AND T0."SlpCode"  = '+cove+
    	    ...
    	    if (rs.length > 0)
        	{
        	    ...
        	    objResult = functions.CreateJSONMessage(-101, "No se han encontrado registros con los parámetros indicados. ("+empId+" - "+functions.GetCurrentDate()+")");
        	    objResponse = functions.CreateResponse(objType, objResult, 0);
        	    functions.DisplayJSON(objResponse, objType);
        	}
	    }else{
    	    objType = "MessageError";
    	    objResult = functions.CreateJSONMessage(-99, "El usuario no tiene un vehiculo asignado. U("+usrId+")");
    	    objResponse = functions.CreateResponse(objType, objResult, 0);
    	    functions.DisplayJSON(objResponse, objType);
    	}
