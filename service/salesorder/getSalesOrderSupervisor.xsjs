...
try{
	...
	{ 
	    ...
	    var emps = ObtenerEmpleados(usrId, dbname);
	    ...
	    if(emps !== undefined){
	        var query = 'SELECT     ' +
                        ...
                        //  '  AND IFNULL(T0."Indicator",\'\') IN (\'01\',\'03\') ' +
                          '  AND T0."SlpCode" <> -1  AND T0."SlpCode"  IN ('+emps+') ' +
                          '  	UNION ' +
                          '  SELECT  ' +
                          '  	\'P\' AS "Tipo",   	 ' +
                          '  	T0."DocEntry"  AS "Clave",   	 ' +
	        ...
	        objType = "MessageError";
    	    objResult = functions.CreateJSONMessage(-99, "El usuario supervisor no tiene empleados asignados.");
    	    objResponse = functions.CreateResponse(objType, objResult, 0);
    	    functions.DisplayJSON(objResponse, objType);
	    }
	}else{
